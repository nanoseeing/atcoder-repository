...
class BoardSimulator {
  ...
  public:
    ...
    std::pair<std::pair<int, int>, std::vector<std::vector<int>>> get_next_coordinate(Env &env) {
        ...
        // double random_index_prob = std::max(1.0 / M * 1.5, 1.0 - (simulate_cnt / (std::pow(N, 2) / 3)));
        double base_cnt = std::pow(N, 2) / 3;
        double random_index_num = std::max(0.1, (base_cnt - simulate_cnt) / base_cnt);
        random_index_num = std::min(1, (int)(M * random_index_num));
