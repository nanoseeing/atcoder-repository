...
vector<int> construct_tour(const vector<Path> &path) {
    ...
    set<int> unique_tour;
    for(int i = 0; i < N; i++) {
        unique_tour.insert(tour[i]);
    }
    assert(unique_tour.size() == N);
    return tour;
}

vector<Path> construct_path(const vector<int> &tour, bool sort_flag = false) {
    vector<Path> path(N);
    for(int i = 0; i < N; i++) {
...
vector<Path> double_bridge(const vector<Path> &path) {
    ...
    auto new_path = construct_path(new_tour, true);
    return new_path;
}
...
vector<Path> two_opt(const vector<Path> &path, int u1, int u2, int v1, int v2) {
    ...
    vector<int> new_tour;
    if(u2 < v1)
        new_tour = two_opt(tour, u2, v1);
    else
        new_tour = two_opt(tour, v2, u1);
    auto new_path = construct_path(new_tour, true);
    return new_path;
}
...
vector<Path> calc_init_ans(const vector<vector<double>> &dist_matrix) {
    ...
    return construct_path(init_x, true);
}
