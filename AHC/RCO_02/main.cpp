...
struct ProblemInput {
    ...
    string to_string(int map_num) const {
        string ret;
        for(int h : views::iota(0, H)) {
            string s = string(map[map_num][h].begin(), map[map_num][h].end());
            ret += s + "\n";
        }
        return ret;
    }
...
struct State {
    ...
    bool isEnd() const {
        return max_turn_ == turn_;
    }
    ull getHash() const {
        return 0;
    }
    pair<int, ull> moveHelper(const Operation &op, bool update) {

        int new_score = score_;
        for(int i : views::iota(0, problem_input_.K)) {
            auto &now_map = problem_input_.map[i];
            int x = player_positions_[i].first;
            int y = player_positions_[i].second;

            int new_x = x;
            int new_y = y;
            if(now_map[x][y] != '@') {
                if(op.direction == 'U' && 0 < y && now_map[x][y - 1] != '#') {
                    new_y = y - 1;
                } else if(op.direction == 'D' && y < problem_input_.H - 1 && now_map[x][y + 1] != '#') {
                    new_y = y + 1;
                } else if(op.direction == 'L' && 0 < x && now_map[x - 1][y] != '#') {
                    new_x = x - 1;
                } else if(op.direction == 'R' && x < problem_input_.W - 1 && now_map[x + 1][y] != '#') {
                    new_x = x + 1;
                }
            }

            if(now_map[new_x][new_y] == 'o') {
                new_score++;
            }
            if(update) {
                now_map[new_x][new_y] = ' ';
                player_positions_[i] = {new_x, new_y};
            }
        }
    ...
    pair<int, ull> tryMove(const Operation &op) {
        for(int i : views::iota(0, problem_input_.K)) {
            if(op.direction == 'U') {
                player_positions_[i].first--;
            } else if(op.direction == 'D') {
                player_positions_[i].first++;
            } else if(op.direction == 'L') {
                player_positions_[i].second--;
            } else if(op.direction == 'R') {
                player_positions_[i].second++;
            }
        }
        return {0, 0};
    ...
    for(int turn = 1; turn <= max_depth; turn++) {
        ...
        for(const int i : views::iota(0, (int)states.size())) {
            ...
            for(auto &op : states[i].getValidOperations()) {
                auto [next_score, next_hash] = states[i].tryMove(op, problem_input);
        ...
        for(const auto &temp_node : temp_nodes) {
            ...
            next_states.back().applyMove(temp_node.op, problem_input);
