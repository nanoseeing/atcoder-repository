...
const double POST_OPT_GRUOP_TIME = 0.5;
const double POST_OPT_T0 = 10.0;
const double POST_OPT_T1 = 0.5;

// グループ推定（オンリー）
const double GROUP_T0 = 10.0;
const double GROUP_T1 = 0.5;
...
neighbor_function_define_group(
    ...
    for(int i = 0; i < G; ++i) {
        weighted_costs.emplace_back(static_cast<double>(costs[i]) / max(1, (int)vs[i].size()));
    }
    ...
    for(int g = 0; g < G; ++g) {
        ...
        if(d > 1e-6) {
            dist_weights.emplace_back(1.0 / d);
    ...
    int g_num = xor_uniform(xor_rng, 1, min({MAX_G_NUM, g1_num, g2_num}));
...
// inline pii get_coord(int vid, int update_vid, const pii &updated_coord, const vector<pii> &x_now) {
//     return (vid == update_vid) ? updated_coord : x_now[vid];
// };
...
calc_cost(const vector<pair<pii, pii>> &constrains,
    ...
    for(const auto &[v, now_coord] : x_update) {
        for(const auto &consid : vid_to_constid[v]) {
            ...
            // int now_cost = cost_formula(
            //     get_coord(p.first, v, now_coord, x_now),
            //     get_coord(p.second, v, now_coord, x_now),
            //     get_coord(q.first, v, now_coord, x_now),
            //     get_coord(q.second, v, now_coord, x_now));
...
pair<vector<pii>, int> simulated_annealing_estimate_coords(
    ...
    while(true) {
        ...
        if(time_div_q.size() > 0 && elapsed >= time_div_q.front()) {
            x_best_memo.emplace_back(x_best);
...
int main() {
    if(IS_ONLINE_JUDGE) {
        ...
        ofstream result_file("../result/cpp_fast.txt");
        result_file << "avg: " << avg << "\n";
