...
int DEBUG_QUERY = -1;
int DEBUG_L = -1;
const int START_FILE_NUM = 62;
const int END_FILE_NUM = 63;
...
int cost_update_arr[10000];
int cost_update_id[10000];
...
// 本体
// =================================
int cost_formula(int d1, int d2, pair<int, int> coord, vector<int> rect) {
    int p1, p2;
    tie(p1, p2) = coord;
    if(p1 == -1 && p2 == -1) {
...
calc_init_cost(const vector<pair<pair<int, int>, pair<int, int>>> &constrains,
    ...
    for(int ci = 0; ci < constrains.size(); ++ci) {
        ...
        cost_list[ci] = cost_formula(dist1, dist2, pair<int, int>(-1, -1), vector<int>());
...
calc_cost(const vector<pair<pair<int, int>, pair<int, int>>> &constrains,
    ...
    for(const auto &[v, _] : x_update) {
        pair<int, int> coord = x_update.at(v);
        vector<int> target_rect = rectangles.at(v);
        for(const auto &consid : vid_to_constid[v]) {
            ...
            // int now_cost = cost_formula(dist1, dist2, x_update.at(v), rectangles.at(v));
            int now_cost = cost_formula(dist1, dist2, coord, target_rect);
...
neighbor(const vector<pair<int, int>> &x,  // 現在の座標
    ...
    // ランダムに k 個サンプリング
    vector<int> vs;
    sample(target_v.begin(), target_v.end(), std::back_inserter(vs), k, gen);
...
pair<vector<pair<int, int>>, int> simulated_annealing(
    ...
    while(true) {
        ...
        tie(cost_new, cost_update_cnt) = calc_cost(
            constrains, x, x_update, cost_list, cost_current, vid_to_constid, rectangles);
