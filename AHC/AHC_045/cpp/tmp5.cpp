...
const int SOLVE_L_THREAD = 0; // < SOLVE_L_THREADなら点推定しない

// 点推定
const double MAX_TIME = 10.62;
...
const double POST_OPT_GRUOP_TIME = 0.1;
...
pair<int, int> calc_centroid(const vector<pair<int, int>> &points) {
...
pair<vector<pair<int, int>>, int> prim_vs(
...
class Env {
  public:
    ...
    vector<pair<int, int>> query_history;
...
class MyGraph {
  public:
    ...
    MyGraph(const vector<int> &vs_, const vector<pair<int, int>> &edges_, int group_ind_)
    ...
    void update_edges(const vector<pair<int, int>> &remove_edges,
                      const vector<pair<int, int>> &add_edges) {
        set<pair<int, int>> current(edges.begin(), edges.end());
    ...
    pair<vector<int>, vector<pair<int, int>>> walk_bfs(int init_v, int q_num) {
        unordered_set<int> visited_v;
        unordered_set<pair<int, int>, pair_hash> visited_edge;
        vector<pair<int, int>> q = {{-1, init_v}};
        ...
        vector<pair<int, int>> visited_edge_list(visited_edge.begin(), visited_edge.end());
...
pair<vector<vector<int>>, vector<vector<pair<int, int>>>> update_mst(
    Env &env,
    vector<vector<int>> &ans_v,
    vector<vector<pair<int, int>>> &ans_edges,
    const unordered_map<int, int> &group_query_cnt) {
    for(int g = 0; g < env.M; ++g) {
        vector<int> &now_v = ans_v[g];
        vector<pair<int, int>> &now_edges = ans_edges[g];
        ...
        if(now_v.size() < 3) {
            ...
            ans_edges[g] = env.query(now_v); // query should return vector<pair<int, int>>
        } else {
...
tuple<
    ...
    vector<vector<pair<int, int>>>, // edges [g1_edges, g2_edges]
    vector<pair<int, int>>          // centroids [g1_centroid, g2_centroid]
    >
neighbor_function_define_group(
    const vector<vector<int>> &vs,
    const vector<int> &costs,
    const vector<pair<int, int>> &centroids,
    ...
    // 重心再計算
    vector<pair<int, int>> g1_coords, g2_coords;
    for(int v : next_vs[g1])
        g1_coords.push_back(coords[v]);
    for(int v : next_vs[g2])
        g2_coords.push_back(coords[v]);

    pair<int, int> g1_centroid = calc_centroid(g1_coords);
    pair<int, int> g2_centroid = calc_centroid(g2_coords);
...
pair<vector<vector<int>>, vector<vector<pair<int, int>>>>
simulated_annealing_define_group(
    ...
    vector<vector<pair<int, int>>> edges = ans_edges;

    // 初期重心計算
    vector<pair<int, int>> centroids;
    for(const auto &vs_list : x) {
        vector<pair<int, int>> target_coords;
        for(int v : vs_list) {
...
// solve 本体
int solve_for_group(Env &env, vector<pair<int, int>> &target_points) {
...
pair<vector<pair<int, int>>, int> simulated_annealing_estimate_coords(
    ...
    vector<vector<pair<int, int>>> x_best_memo;
...
vector<vector<int>> calc_qeury_target(Env &env) {
...
int solve_coord_estimate(Env &env) {
    ...
    // const auto query_target = calc_qeury_target(env);
...
int main() {
    if(IS_ONLINE_JUDGE) {
        ...
        ofstream result_file("../result/cpp_total_update01.txt");
        result_file << "avg: " << avg << "\n";
