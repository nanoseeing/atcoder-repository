...
// グラフアルゴリズム
// =================================
pair<unordered_set<int>, unordered_set<int>> cut_graph(
    const unordered_map<int, vector<int>> &graph,
    pair<int, int> cut_edge) {
    auto bfs = [](const unordered_map<int, vector<int>> &graph, int init_v, int not_visit_v) {
...
vector<vector<int>> construct_dist_matrix(const vector<pair<int, int>> &points) {
    int N = points.size();
...
unordered_map<int, vector<pair<int, int>>> construct_graph(const unordered_map<int, pair<int, int>> &points) {
...
unordered_map<int, vector<int>> construct_graph_from_edges(
...
vector<tuple<int, int, int>> construct_sorted_edges(
    const unordered_map<int, pair<int, int>> &coords) {
...
// クラスカル法による最小全域木
pair<vector<pair<int, int>>, int> kruskals_algorithm(vector<tuple<int, int, int>> edges, const vector<int> &vs) {
    UnionFind uf(vs);
    int cost = 0;
...
tuple<vector<pair<int, int>>, vector<int>, int> prim_k(
    const vector<vector<int>> &graph,
    int init_v,
    int k,
    const unordered_set<int> &used_v,
    int N) {
...
class Env {
  public:
    ...
    virtual int answer(const vector<vector<int>> &groups, const vector<vector<pair<int, int>>> &edges) = 0;
...
class EnvOnline : public Env {
  public:
    ...
    int answer(const vector<vector<int>> &groups, const vector<vector<pair<int, int>>> &edges) override {
        cout << "!" << endl;
...
class EnvOffline : public Env {
  ...
  public:
    EnvOffline(const string &input_path, const string &output_path, bool dbg = false, int dbg_L = -1, int dbg_Q = -1)
...
tuple<vector<int>, int> calc_init_cost(
    ...
    for(int ci = 0; ci < constrains.size(); ++ci) {
        ...
        int dist1 = calc_dist(x[p1], x[p2]);
        int dist2 = calc_dist(x[q1], x[q2]);
...
tuple<unordered_map<int, int>, int> calc_cost(
    const vector<pair<pair<int, int>, pair<int, int>>> &constrains,
    const vector<pair<int, int>> &x_now,
    const unordered_map<int, pair<int, int>> &x_update,
    const vector<int> &cost_list,
    int cost_now,
    const vector<vector<int>> &vid_to_constid) {
    int new_cost = cost_now;
    unordered_map<int, int> cost_update;
    cost_update.reserve(500);

    for(const auto &[v, _] : x_update) {
        for(const auto &consid : vid_to_constid[v]) {
            auto [p, q] = constrains[consid];
            auto [p1, p2] = p;
            auto [q1, q2] = q;

            const auto get_coord = [&](int vid) -> const pair<int, int> & {
                auto it = x_update.find(vid);
                if(it != x_update.end())
                    return it->second;
                return x_now[vid];
            };

            const auto &coord1 = get_coord(p1);
            const auto &coord2 = get_coord(p2);
            const auto &coord3 = get_coord(q1);
            const auto &coord4 = get_coord(q2);

            int dist1 = calc_dist(coord1, coord2);
            int dist2 = calc_dist(coord3, coord4);

            int pre_cost = cost_list[consid];
            int now_cost = cost_formula(dist1, dist2);
...
pair<unordered_map<int, pair<int, int>>, double> neighbor(
    const vector<pair<int, int>> &x,  // 現在の座標
...
pair<vector<pair<int, int>>, int> simulated_annealing(
    ...
    while(true) {
        ...
        int cost_new;
        tie(cost_update, cost_new) = calc_cost(
        ...
        if(delta_cost <= 0 || rand01(gen) < exp(-delta_cost / temp)) {
            ...
            for(const auto &[ci, val] : cost_update) {
                cost_list[ci] = val;
            }
        }
        ...
        simulate_cnt++;
        if(display && simulate_cnt % 10000 == 0) {
            debug_print("i: ", simulate_cnt,
...
calc_greedy_answer(Env &env, const vector<pair<int, int>> &target_points) {
    ...
    for(const auto &[group_n, group_size] : groups) {
        ...
        vector<int> prim_v;
        double prim_cost;
        tie(prim_edges, prim_v, prim_cost) = prim_k(graph, v, group_size, now_used, env.N);
...
int solve(Env &env) {
    ...
    sort(max_d_list.begin(), max_d_list.end(), [](auto &a, auto &b) {
    ...
    for(int i = 0; i < env.Q && i < max_d_list.size(); ++i) {
        ...
        other_points.resize(min(env.L * 2, (int)other_points.size()));

        vector<int> other_points_ind;
        for(auto &[d, p] : other_points)
            other_points_ind.emplace_back(p);

        sort(other_points_ind.begin(), other_points_ind.end(), [&](int a, int b) {
            return max_d_points[a] > max_d_points[b];
        });
        ...
        tmp_random_vs.insert(tmp_random_vs.end(), other_points_ind.begin(), other_points_ind.end());
    ...
    vector<pair<pair<int, int>, pair<int, int>>> constrains_list(constrains.begin(), constrains.end());
    ...
    for(int trial = 0; trial < TRIAL_NUM; ++trial) {
        auto [x_best, cost_best] = simulated_annealing(
            env.center_points, 1.0, 0.1, MAX_TIME, 0.5,
            constrains_list, env.rectangles, random_vs_list,
            vid_to_constid, true);
...
int main() {
    if(IS_ONLINE_JUDGE) {
        ...
        for(int file_num = 19; file_num < 20; ++file_num) {
            debug_print("=====");
            ...
            debug_print("[", file_num, "/", FILE_NUM, "]: ", cost, " ", fixed, setprecision(2), elapsed, "s");
        ...
        ofstream result_file("../result/mcmc10.txt");
        result_file << "avg: " << avg << "\n";
