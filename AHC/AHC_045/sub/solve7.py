...
def compute_knn_density(inds, points, k: int) -> int:
    tree = KDTree(points)
    min_dentist = float("inf")
    for ind, point in zip(inds, points):
        distances, _ = tree.query(point, k=k + 1)
        k_sum = sum([distance**2 for distance in distances])
        density = k / (k_sum + 1e-8)
        if density < min_dentist:
            min_dentist = density
            ret_ind = ind
    return ret_ind
    ...
    def query(self, c_list: list[int]) -> list[tuple[int, int]]:
        ...
        for c1 in c_list:
            for c2 in c_list:
                if c1 == c2:
                    continue
                dist = calc_dist(self.coordinates[c1], self.coordinates[c2])
...
def calc_greedy_answer(env: EnvOffline, target_points):
    ...
    for group_n, group_size in groups:
        not_used_list = list(not_used)
        now_points = [target_points[i] for i in not_used_list]
        v = compute_knn_density(not_used_list, now_points, group_size)
        # v = not_used.pop()
...
class MyGraph:
    ...
    def cut_graph(self, edge_id):
        ...
        group1 = self.bfs(v1, target_edge)
        group2 = set(self.vs) - group1
        return v1, v2, group1, group2

    def bfs(self, init_v, target_edge):
        target_edge_set = set(target_edge)
        visited = set([init_v])
        q = deque([init_v])
        while q:
            pv = q.pop()
            for nv in self.graph[pv]:
                if nv not in visited and (set([nv, pv]) != target_edge_set):
                    visited.add(nv)
                    q.append(nv)
        return visited
...
def update_mst(env: EnvOffline, ans_v, ans_edges):
    ...
    INF = 10**18
    for _ in range(env.Q - query_cnt):
        min_cost = INF
        target_group = None
    ...
    for g in range(env.M):
        ...
        for _ in range(group_query_cnt[g]):
            if 80 <= len(now_edges):
                eis = random.sample(range(len(now_edges)), 80)
            else:
                eis = list(range(len(now_edges)))
            for ei in eis:
