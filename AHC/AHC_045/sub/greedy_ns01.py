...
class EnvOnline(Env):
    def __init__(self):
        ...
        self.read_second_line([input() for _ in range(self.N)])
...
def greedy_ns_prim(env: Env):
    ...
    return sorted_ans_vs, sorted_ans_edges
...
def solve(env: Env):
    anv_v_estimate, ans_edges_estimate = greedy_ns_prim(env)
    for g in range(env.M):
        assert len(set(anv_v_estimate[g])) == env.G[g]
        assert len(set(ans_edges_estimate[g])) == env.G[g] - 1
...
def main():
    if IS_ONLINE_JUDGE:
        ...
        with open("../result/greedy_ns.txt", "w") as f:
            f.write(f"avg: {avg}\n")
