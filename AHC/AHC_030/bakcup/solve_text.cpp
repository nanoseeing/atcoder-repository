...
class BoardSimulator {
  ...
  public:
    ...
    BoardSimulator(double _diff_pow, double _cost_pow) : simulate_avg_num(0), simulate_cnt(0), diff_pow(_diff_pow), cost_pow(_cost_pow) {}
    ...
    double _calc_cost_from_simulated_board(Env &env, const std::vector<std::vector<int>> &simulated_board) {
        ...
        for (int y = 0; y < N; ++y) {
            for (int x = 0; x < N; ++x) {
                ...
                // double v = std::pow(std::abs(simulated_board.at(y).at(x) - f_board.at(y).at(x)), diff_pow);
            ...
            for (int y = 0; y < N; ++y) {
                for (int x = 0; x < N; ++x) {
                    if (now_board.at(y).at(x) > 0) {
                        // double center_cost = std::max(0.0, (std::abs(x - N - 1.0) + std::abs(y - N - 1.0)) / (N - 1.0) * 0.25);
...
void solve(Env &env, double diff_pow, double cost_pow) {
    BoardSimulator board_simulator(diff_pow, cost_pow);
    for (int i = 0; i < 500; ++i) {
        if (env.is_ok()) {
...
int main() {
    ...
    double mean_score = 0;
    // double diff_pow = std::stod(argv[1]);
    // double cost_pow = std::stod(argv[2]);
    double diff_pow = 1.0;
    double cost_pow = 1.0;
    std::srand(0);
    for (int test_num = 0; test_num < TEST_NUM; ++test_num) {
        ...
        Env env(file_path);
        solve(env, diff_pow, cost_pow);
        std::cout << "score:" << env.score << std::endl;
        mean_score += env.score;
    }
    mean_score = mean_score / TEST_NUM;
