...
def greedy(N, known_grid=set()):
    has_oil = []
    for x in range(N):
        for y in range(N):
            if (x, y) in known_grid:
                continue
            resp = query1(x, y)
            if resp != 0:
                has_oil.append((x, y))
...
def my_greedy(N, ESP):
    ...
    for i in range(not_greedy_n):
        if query1(*coordinates[-1 - i]) > 0:
            define_oils.append(coordinates[-1 - i])

    resp = answer(has_oils + define_oils)
    return resp == "1", define_oils
...
def main():
    ...
    ok, define_oils = my_greedy(N, ESP)
    if ok:
        return
    greedy(N, define_oils)
