...
def my_greedy(N, M, ESP, GRID, response_simulator: ResponseSimulator):
    ...
    for i in range(0, greedy_n, query_num + 1):
        ...
        coordinate_combs = itertools.combinations(query_indexs, query_num)
        ...
        for q in query_indexs:
            tmp_v = 0
            for comb in coordinate_combs:
                if q in comb:
                    tmp_v += query_vals[comb]
                else:
                    tmp_v -= query_vals[comb]
            v = tmp_v // query_num
            total += v
            if v > 0:
                has_oils.append(coordinates[i + q])
        if total == total_grid:
            return answer(has_oils)
        if total > total_grid:
            return False
...
def main():
    ...
    response_simulator.env_setting(query_num=best_query_num)
    if my_greedy(N, M, ESP, GRID, response_simulator):
        return
    else:
        greedy(N, M, ESP, GRID)
