...
public:
    ...
    bool answer(const std::vector<std::pair<int, int>>& coordinates) {
        bool is_ok = total_oil_num == std::accumulate(coordinates.begin(), coordinates.end(), 0,
            [this](int sum, const std::pair<int, int>& coord) {
    ...
    int query1_count() const {
        return queries1.size();
    }
};
...
public:
    ...
    std::pair<std::pair<int, int>, std::vector<std::vector<int>>> simulate_board() {
        ...
        for (int simulate_num = 0; simulate_num < SIMULATE_NUM; ++simulate_num) {
            ...
            for (int m = 0; m < M; ++m) {
                int sampling_index = placement_indexes[rand() % placement_indexes.size()].first;
                for (const auto& coord : grids[m]) {
                    int ny = coord.first + sampling_index;
                    int nx = coord.second;
                    now_board[ny][nx] += 1;
                }
            }
...
int main() {
    ...
    for (int test_num = 0; test_num < TEST_NUM; ++test_num) {
        ...
        Env env("./in/" + std::to_string(test_num) + ".txt");
        solve(env);
