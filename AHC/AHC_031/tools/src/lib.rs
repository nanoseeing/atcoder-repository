...
pub fn compute_score_details(
    ...
    let mut area_score = 0;
    let mut partial_area_score = 0;
    ...
    for d in 0..out.len() {
        ...
        for k in 0..input.N {
            ...
            let b = (i1 - i0) * (j1 - j0);
            if input.a[d][k] > b {
                area_score += 100 * (input.a[d][k] - b) as i64;
            }
            for j in j0..j1 {
                if i0 > 0 {
                    hs2.insert((i0, j));
    ...
    let score = (area_score + partial_area_score) + 1;
    (score, String::new(), change)
}
                ...
                .add(
                    rect(
                        ...
                        &if input.a[out.len() - 1][k] > b {
                            format!(
                                "#ff0000{:02x}",
                                ((input.a[out.len() - 1][k] - b) * 5).min(255)
                            )
                        } else {
                            "white".to_owned()
                        },
