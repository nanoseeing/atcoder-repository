...
ll help(vector<int> group) {
    ...
    while(true) {
        auto next = a_new[start];
        if(v_set.contains(next)) {
            circle_start = next;
            break;
        }
        v_set.insert(next);
    ...
    cpp_dump(visited, v_set);
    assert(circle_start != -1);

    vll dp(n, -1);
    ll s = circle_start;
    unordered_set<int> tmp_v;
    tmp_v.insert(s);
    while(true) {
        ll next = a_new[s];
        if(tmp_v.contains(next)) {
            break;
        } else {
            tmp_v.insert(next);
            s = next;
