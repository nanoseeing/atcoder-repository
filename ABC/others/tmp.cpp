...
public:
    CumulativeSum2D(int h, int w, T v = 0) {
        H = h;
        W = w;
        vector<vector<T>> init_arr(H + 1, vector<T>(W + 1, v));
        arr = init_arr;
    }
    ...
    void build() {
        for(int h : views::iota(0, H + 1)) {
            for(int w : views::iota(0, W)) {
                arr[h][w + 1] += arr[h][w];
            }
        }

        for(int w : views::iota(0, W + 1)) {
...
void solve() {
    ...
    vector<int> Y(N), X(N);
    unordered_set<pair<int, int>> holes;
    CumulativeSum2D<ll> cum(H, W);

    for(int i : range(N)) {
        cin >> Y[i] >> X[i];
        Y[i]--, X[i]--;
        holes.insert({Y[i], X[i]});
        cum.add(Y[i], X[i], 1);
    }
    ...
    for(int h : range(H)) {
        for(int w : range(W)) {
            if(holes.contains({h, w})) continue;
            int max_size = min(H - h, W - w);
            auto f = [&](int shift) { return cum.query(w, w + shift, h, h + shift) == 0 ? true : false; };
            int cnt = meguru_search(0, max_size + 1, f);
            ans += cnt;
        }
    }
