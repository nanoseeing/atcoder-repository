...
void solve() {
    ...
    vvi ans(H - h + 1, vi(W - w + 1));

    for(int y : range(H - h + 1)) {
        auto tmp_cnt = map_cnt;
        int cnt = map_cnt.size();
        for(int x : range(W - w + 1)) {
            if(x == 0) {
                for(int dy : range(h)) {
                    for(int dx : range(w)) {
                        if(tmp_cnt[A[y + dy][x + dx]] == 1) cnt--;
                        tmp_cnt[A[y + dy][x + dx]]--;
                    }
                }
            } else {
                for(int dy : range(h)) {
                    if(tmp_cnt[A[y + dy][x - 1]] == 0) cnt++;
                    tmp_cnt[A[y + dy][x - 1]]++;
                }
                for(int dy : range(h)) {
                    if(tmp_cnt[A[y + dy][x + w - 1]] == 1) cnt--;
                    tmp_cnt[A[y + dy][x + w - 1]]--;
                }
            }
            ans[y][x] = cnt;
            cpp_dump(y, x, tmp_cnt);
        }
    }
