...
// dijkstra O(MlogM)
template <integral T>
pair<vector<T>, vector<int>> dijkstra(const Graph<T> &graph, int s) {
...
void solve() {
    ...
    for(int i : range(M)) {
        ...
        graph[v + N].push_back({u + N, 1LL});
    }
    for(int v : range(N)) {
        graph[v].push_back({v + N, X});
        graph[v + N].push_back({v, X});
    }
    auto [dp, prev] = dijkstra(graph, 0);
    cout << min(dp[N - 1], dp[2 * N - 1]) << endl;
}
